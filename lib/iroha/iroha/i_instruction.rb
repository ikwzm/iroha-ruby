module Iroha
  ...
  class IInstruction
    ...
    def _to_exp(indent)
      ...
      input_regs  = "(" + @_input_registers .map{|id| @_owner_table._find_register(id)._id}.join(" ") + ")"
      output_regs = "(" + @_output_registers.map{|id| @_owner_table._find_register(id)._id}.join(" ") + ")"
